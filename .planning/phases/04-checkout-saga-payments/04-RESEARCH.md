...
1. **Does the ASB emulator actually deliver scheduled messages?** — GATED, not yet resolved.
   ...
   - **Status: RESOLVED.** Task 2 (`spikes/04-asb-scheduling-spike/`) drove the real `mcr.microsoft.com/azure-messaging/servicebus-emulator` image directly via `Azure.Messaging.ServiceBus`'s native `ScheduledEnqueueTime`, decoupled from the saga/HTTP/auth stack. Its `<verify><automated>` step required `SPIKE-RESULT: PASS` on stdout before Task 3's ASB-native `Schedule`/`Unschedule` implementation could be treated as validated.
   - **Resolution:** `SPIKE-RESULT: PASS — message was delivered on or after the scheduled time; ASB emulator scheduled delivery is confirmed.` Observed 2026-08-08 by running `docker compose -f spikes/04-asb-scheduling-spike/docker-compose.yml up -d`, waiting for the emulator's `Emulator Service is Successfully Up!` log line, then `dotnet run --project spikes/04-asb-scheduling-spike/SpikeRunner.csproj`, which scheduled a message 10 seconds in the future and received it only after that time (message did not arrive early, and did arrive within the 30-second wait window). Docker resources were torn down afterward (`docker compose ... down -v`). The ASB-native `Schedule`/`Unschedule` approach is confirmed viable against the local emulator — Task 3 proceeds on this basis; the Quartz.NET-for-both-environments fallback is not needed.
...
| Azure Service Bus emulator (Docker) | CHK-05 timeout scheduling, all pub/sub in this phase | Confirmed available and scheduled-delivery-capable — 04-02-PLAN.md Task 2's standalone spike (`spikes/04-asb-scheduling-spike/`) ran the real emulator image on 2026-08-08 and observed `SPIKE-RESULT: PASS` (see Open Question 1 resolution) | `mcr.microsoft.com/azure-messaging/servicebus-emulator:latest` | Quartz.NET-for-both-environments (see Alternatives Considered) — not needed, spike passed |
