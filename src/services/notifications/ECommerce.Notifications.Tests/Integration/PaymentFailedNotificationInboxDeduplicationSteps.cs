...
public class PaymentFailedNotificationInboxDeduplicationSteps : IAsyncLifetime
{
    ...
    public async Task When_SamePaymentFailedPublishedTwice(
        Guid messageId, Guid checkoutId, string userId, decimal amount, string reason, DateTimeOffset failedAt)
    {
    ...
    public async Task Then_NotificationEntriesHasExactlyOneRow()
    {
        ...
        notificationCount.Should().Be(1,
            "a redelivered PaymentFailed (same transport MessageId) must produce exactly one NotificationEntry row, not two (NOT-02)");
    }
}
